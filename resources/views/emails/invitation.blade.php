...
<b>{{$user->first_name}} {{$user->last_name}}</b> has invited you to join us here at @if($siteTitle=App\Helpers\SiteConfigurationHelper::getConfigurationAttr()->title_text){{$siteTitle}}@else Estate Baron @endif <br> 
@if($siteTitle=App\Helpers\SiteConfigurationHelper::getConfigurationAttr()->title_text){{$siteTitle}}@else Estate Baron @endif provides investment opportunities to all investor types.<br><br>
