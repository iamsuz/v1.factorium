											...
											<div class="caption">
												...
												<a @if($project->is_coming_soon) @if(Auth::user())
													...
													<div class="text-left" @if($invoice_sold == '1' || $invoice_sold == '2') style="margin-top: 5.7rem;" @endif>
														...
														<div class="col-xs-4 col-sm-4 col-md-4 text-center" data-wow-duration="1.5s" data-wow-delay="0.6s" style="@if(!$project->projectconfiguration->show_duration) display:none; @endif border-left: thin solid #000; padding: 0 5px;" ><h4 class="first_color" style="color:#282a73;margin-top:1px;margin-bottom:1px;font-size:22px;">@if(!$invoice_sold)<span class="invoice-due-date" id="invoice_due_date_{{ $project->id }}" data-project-id="{{ $project->id }}" data-due-date="@if(!$invoice_sold) {{ $project->investment->fund_raising_close_date }} @endif"> @if($project->investment){{$project->investment->remaining_hours}}@endif</span>@else <span class="invoice-sold-date" id="invoice_sold_date_{{ $project->id }}" data-project-id="{{ $project->id }}" data-due-date="{{ $project->investment->fund_raising_close_date }}"data-sold-date="@if($invoice_sold == 1){{ $project->soldInvoice->first()->share_certificate_issued_at }}@elseif($invoice_sold == 2) @if($project->repurchased->first()){{ $project->repurchased->first()->share_certificate_issued_at }}@else{{ $project->repurchased_by_partial_pay->first()->share_certificate_issued_at }}@endif @endif"></span>@endif<small><small><br>Days</small></small></h4>
														</div>
